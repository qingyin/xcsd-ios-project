...
- (void)setDefaultValues {
    ...
    _legendView = [[JYLegendView alloc] initWithFrame:CGRectMake(self.width_ - 120, 35, 80, 50)];
...
//	_numOfV = [_dataSeries[0] count];
    NSArray *tmpArr = _dataSeries[0];
    _numOfV = tmpArr.count;
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat padding =  -7;
    CGFloat yPadding = -7;
    NSArray *tmpArr = self.dataSeries.firstObject;
    
	for (int i = 0; i < _numOfV; i++) {
        
        NSNumber *value = tmpArr[i];
...
//		CGSize attributeTextSize = JY_TEXT_SIZE(attributeName, self.scaleFont);
		...
		CGFloat yOffset = pointOnEdge.y > _centerPoint.y ? height / 2.0 + yPadding : -height / 2.0 - yPadding;
        
        if (pointOnEdge.x == _centerPoint.x) {
...
//                       withAttributes:attributes];
            ...
            ResultLabel *resultlbl = [[ResultLabel alloc] initWithFrame:CGRectMake(legendCenter.x - width / 2.0, legendCenter.y - height / 2.0, width, height)];
            [resultlbl setTitle:[NSString stringWithFormat:@"%@",value] str:attributeName color:self.eachLineColors[i]];
            [self addSubview:resultlbl];
        }
        else {
//            [attributeName drawInRect:CGRectMake(legendCenter.x - width / 2.0,
//                                                 legendCenter.y - height / 2.0,
...
//                            alignment:NSTextAlignmentCenter];
    ...
    [[UIColor colorWithRed:229 / 255.0 green:229 / 255.0 blue:229 / 255.0 alpha:1.0f] set];
	...
	for (int i = 0; i < _numOfV; i++) {
        ...
        UIColor *color = self.eachLineColors[i];
...
//        [RGBCOLOR(145, 145, 145) set];
		for (int step = 0; step <= _steps; step++) {
            ...
            [currentLabel drawInRect:CGRectMake(_centerPoint.x + 3,
                                               ...
                                               20,
                                                10) withAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:10], NSForegroundColorAttributeName : self.stepTextColor}];
		}
	}
}
