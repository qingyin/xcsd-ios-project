...
- (void)initHeaderView
{
    ...
    [self.contents setTitleColor:KColorAppMain forState:UIControlStateNormal];
    ...
    self.markView.backgroundColor = KColorAppMain;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if (self.scrollView.contentOffset.x == 0) {
        [self.descrip setTitleColor:KColorAppMain forState:UIControlStateNormal];
        ...
        [self.descrip setTitleColor:kColorBtn forState:UIControlStateNormal];
        [self.contents setTitleColor:kColorBtn forState:UIControlStateNormal];
        [self.assessment setTitleColor:KColorAppMain forState:UIControlStateNormal];
    }
...
- (void)clickEdit:(UIButton *)sender
{
    ...
    if (infoVC.resourceList.count>0 && self.starNum == 0 && infoVC.videoView.lable.hidden && player.playbackStatus == AUMediaPlaybackStatusPlaying) {
...
//    __weak typeof(self) tmpObj = self;
    // by mey
    __weak __typeof(&*self) tmpObj=self;
...
{
    ContentsCell *cell = (ContentsCell *)[tableView cellForRowAtIndexPath:indexPath];
    cell.titleLable.textColor = KColorAppMain;
    cell.lable1.textColor = KColorAppMain;
}
//点击视频次数
