...
@interface TXAVAssetExportSession()
...
@implementation TXAVAssetExportSession
...
{
    return [TXAVAssetExportSession.alloc initWithAsset:asset];
}
...
- (AVMutableVideoComposition *)buildDefaultVideoComposition
{
    ...
    float offsetX = _cropRatio.x * naturalSize.width;
    float offsetY = _cropRatio.y * naturalSize.height;
    
    CGAffineTransform matrix = CGAffineTransformMakeTranslation(-offsetX, -offsetY);
    CGAffineTransform scaleMatrix = CGAffineTransformConcat(matrix, CGAffineTransformMakeScale(ratio / xratio, ratio / yratio));
    transform = CGAffineTransformConcat(transform, scaleMatrix);
    ...
    if (CMTIME_IS_VALID(self.timeRange.duration) && !CMTIME_IS_POSITIVE_INFINITY(self.timeRange.duration))
    {
        passThroughInstruction.timeRange = CMTimeRangeMake(self.timeRange.start, self.timeRange.duration);
    }
    else
    {
