...
- (id)initWithHolidays:(NSArray *)holidays{
    self = [super init];
    if (self) {
...
-(void)LunarForSolarYear:(CalendarDayModel *)calendarDay{
    int64_t timeSp = ([calendarDay.date timeIntervalSince1970] + 23 * 3600 + 59 * 60) * 1000;

    if ([_holidays containsObject:@(timeSp)]) {
        calendarDay.isHoliday = YES;
    }else{
        calendarDay.isHoliday = NO;
    }
