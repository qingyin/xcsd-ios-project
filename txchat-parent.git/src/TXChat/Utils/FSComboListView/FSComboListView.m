...
#define COMBOX_VIEW_MAX_WIDTH 600.0f
 ...
 #define TitleChangeColor    [UIColor colorWithRed:194/255.0 green:237/255.0 blue:255/255.0 alpha:1]
...
- (void)initialize
{
    ...
    _pickerView.backgroundColor=TitleChangeColor;
    _pickerView.alpha=.3;
...
//    [self.pickerView selectRow:[self.values indexOfObject:[self currentValue]] inComponent:0 animated:NO];
    ...
    the_label.font = [UIFont systemFontOfSize:18 weight:10];
    the_label.textColor=kColorNavigationTitle;
    the_label.textAlignment = NSTextAlignmentCenter;
    [self addSubview:the_label];
    [the_label.layer setBorderColor:[UIColor blackColor].CGColor];
    [the_label.layer setBorderWidth:0];
    [the_label.layer setCornerRadius:10];
    the_label.backgroundColor = [UIColor clearColor];
    _valueLabel = the_label;
    
    // arrow
    UIImage *icon = [UIImage imageNamed:@"LearnWork_triangle_n"];
    //icon = [icon imageWithTintColor:[UIColor whiteColor]];
    _arrowImgV = [[UIImageView alloc] initWithImage:icon];
    _arrowImgV.frame = CGRectMake(frame.size.width-COMBOBOX_HEIGHT-3, 10, COMBOBOX_HEIGHT-8, COMBOBOX_HEIGHT-17);
    ...
    if (self) {
        ...
        for (NSString* option in _values) {
            ...
            CGRect rect = [option boundingRectWithSize:CGSizeMake(MAXFLOAT, 40)
                                                             ...
                                                             attributes:@{NSParagraphStyleAttributeName: paragraphStyle,NSFontAttributeName: [UIFont systemFontOfSize:16]}
                                                                context:nil];
