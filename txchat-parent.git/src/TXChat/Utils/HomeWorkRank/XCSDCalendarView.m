...
@end
@implementation XCSDCalendarView
...
-(void)initViews
{
    ...
    for (NSInteger i = 0; i < 6; i++) {
        XCSDAttendanceWeekView *weekView = [[XCSDAttendanceWeekView alloc] init];
...
-(UIView *)getLastVisibleWeekView
{
    ...
    {
        XCSDAttendanceWeekView *weekView = (XCSDAttendanceWeekView *)_weekViewsArray[i];
...
-(void)refreshViews:(NSArray *)weekInfos
{
    ...
    _weekInfo = weekInfos;
    for (XCSDCalendarWeekModel *index in _weekInfo) {
        [_weekViewsArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            XCSDAttendanceWeekView *weekView = (XCSDAttendanceWeekView *)obj;
