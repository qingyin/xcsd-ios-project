...
@property (nonatomic, assign) BOOL isStartTest;
...
//    NSArray *averArr = [averStr componentsSeparatedByString:@","];
    ...
    self.dict = [NSMutableDictionary dictionaryWithDictionary:@{ K_nameStr : nameArr,
...
- (void)fetchData{
    ...
    @weakify(self);
    [[TXChatClient sharedInstance] HomeworkResult:[self getCurrentChildInfo] onCompleted:^(NSError *error, XCSDPBAbilityStatResponse *abilityDetails) {
...
- (void)updateDataWithDict{
    ...
    NSArray *nameArr = self.dict[K_nameStr];
    NSArray *selfLevelArr = self.dict[K_selfLevelStr];
    NSArray *averageLevelArr = self.dict[K_averageLevelStr];
    NSArray *percentArr = self.dict[K_percentStr];
    NSArray *ScoreArr = self.dict[K_scoreStr];
    NSArray *pointXArr = self.dict[k_pointsXStr];
    NSArray *pointsArr = self.dict[k_pointsStr];
    NSString *totalAbilityLevel = self.dict[k_totalAbilityLevel];
    NSString *abilityQuotient = self.dict[k_abilityQuotient];
    NSString *maxAbilityQuotient = self.dict[k_maxAbilityQuotient];
    ...
    if (abilityQuotient.integerValue == 0) {
        self.imageScoreLbl.text = @"快去测试板块做学习能力测试吧!";
    }
