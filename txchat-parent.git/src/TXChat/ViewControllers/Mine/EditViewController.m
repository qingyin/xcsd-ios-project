...
//        __weak typeof(self)tmpObject = self;
        ...
        TXUser *child = [[TXChatClient sharedInstance] getUserByUserId:user.childUserId error:nil];
        self.tmpName = [user getGuarDerStr];
        child.guarder =[_textField.text trim];
        
        [[TXChatClient sharedInstance] updateUserInfo:child onCompleted:^(NSError *error) {
            [TXProgressHUD hideHUDForView:tmpObject.view animated:YES];
            if (error) {
                child.guarder = tmpObject.tmpName;
