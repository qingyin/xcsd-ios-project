...
@property(nonatomic, strong)NSMutableArray *currentUserCards;
...
- (void)viewDidLoad {
    ...
    self.currentUserCards = [NSMutableArray array];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil)
    {
        ...
        UIImageView *portraitImgView = [[UIImageView alloc] initWithFrame:CGRectMake(kEdgeInsetsLeft, (KCellHight-40.0f)/2, 40, 40)];
        portraitImgView.tag = kCellContentViewBaseTag;
        portraitImgView.layer.cornerRadius = 8.0/2.0f;
    ...
    [portraitImgView TX_setImageWithURL:[NSURL URLWithString:[info[@"img"] getFormatPhotoUrl:40 hight:40]] placeholderImage:[UIImage imageNamed:@"userDefaultIcon"]];
    titleLb.text = info[@"name"];
    NSString *codeStr = info[@"code"];
    NSNumber *parentId = info[@"parentId"];
    if(user.userId == [parentId longLongValue])
    {
        codeLb.text = codeStr.length?@"已绑定":@"未绑定";
    }
    else
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        EditGuardianViewController *editVC = [[EditGuardianViewController alloc] initWithDetailDic:self.currentUserCards];
...
//    __weak typeof(self)tmpObject = self;
    ...
    [[TXChatClient sharedInstance] fetchBindCards:^(NSError *error, NSArray *txpbBindCardInfos) {
        ...
        if (error) {
            ...
            tmpObject.listArr = [NSMutableArray arrayWithCapacity:1];
            tmpObject.currentUserCards = [NSMutableArray arrayWithCapacity:1];
            [txpbBindCardInfos enumerateObjectsUsingBlock:^(TXPBBindCardInfo *info, NSUInteger idx, BOOL *stop) {
                ...
                [dic setValue:[NSNumber numberWithInteger:info.parentId] forKey:@"parentId"];
                if (user.userId == info.parentId) {
                    if([tmpObject.currentUserCards count] == 0) //列表中只出现一次
                    {
                    ...
                    [tmpObject.currentUserCards addObject:dic];
                }else{
                    [tmpObject.listArr addObject:dic];
                }
            }];
