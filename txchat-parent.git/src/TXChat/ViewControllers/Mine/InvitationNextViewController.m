...
//    __weak typeof(self)tmpObject = self;
    ...
    [callListBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
        [[TXSystemManager sharedManager] CheckAddressBookAuthorization:^(bool isAuthorized) {
            if(isAuthorized)
            {
...
//                                                          animated:YES];
            ...
            {
                [tmpObject showAddressPermissionDeniedAlert];
            }
        }];
    ...
    NSString *targetType = [NSString getParentTypeStr:_type];
    TXUser *childUser = [[TXChatClient sharedInstance] getUserByUserId:user.childUserId error:nil];
    
    
    if(childUser && childUser.nickname && childUser.nickname.length > 0)
    {
        _phoneTextField.placeholder = [NSString stringWithFormat:@"输入%@%@的手机号",childUser.nickname, targetType];
    }
    else
    {
    ...
    _codeTextField.placeholder = @"输入上面手机号收到的验证码";
    ...
    if(childUser.nickname && childUser.nickname && childUser.nickname.length > 0)
    {
        _pwdTextField.placeholder = [NSString stringWithFormat:@"请为%@%@设置6-16位密码",childUser.nickname, targetType];
    }
    else
    {
...
- (void)onClickBtn:(UIButton *)sender{
    if (sender.tag == TopBarButtonLeft) {
        ...
        }else if (_pwdTextField.text.length < KMinPasswordLen) {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField.tag == kCodeTextFieldTag) {
        ...
        return YES;
    }else if (textField.tag == kPhoneTextFieldTag && string.length){
