...
static NSString *const kAttendanceUrlString = @"http://123.57.43.111/p/attendance/children.do";
...
//        requestUrl = [NSString stringWithFormat:@"%@p/attendance/children.do",baseUrl];
                                  ...
                                  headerDict:(NSDictionary *)dict
{
    ...
    if (query == nil) {
        urlString = [urlString stringByAppendingString:[NSString stringWithFormat:@"?childId=%@",@(currentUser.childUserId)]];
    }else{
        __block BOOL isExistChildId = NO;
        if (IOS8AFTER) {
            NSURLComponents *urlComponents = [NSURLComponents componentsWithString:urlString];
            NSArray *queryItems = [urlComponents queryItems];
            [queryItems enumerateObjectsUsingBlock:^(NSURLQueryItem *obj, NSUInteger idx, BOOL *stop) {
                if (obj.name && [obj.name isEqualToString:@"childId"]) {
                    isExistChildId = YES;
                    *stop = YES;
                }
            }];
            ...
            [queryList enumerateObjectsUsingBlock:^(NSString *param, NSUInteger idx, BOOL *stop) {
                NSArray *elts = [param componentsSeparatedByString:@"="];
                if ([elts count] == 2) {
                    NSString *itemKey = [elts firstObject];
                    if (itemKey && [itemKey isEqualToString:@"childId"]) {
                        isExistChildId = YES;
                    }
                }
            }];
