...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [btn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            avc.userId = _feed.userId;
            avc.portraitUrl = _feed.userAvatarUrl;
            avc.nickName = _feed.userNickName;
...
- (void)setFeed:(TXFeed *)feed{
    ...
    //头像
    [_portraitBtn TX_setImageWithURL:[NSURL URLWithString:[feed.userAvatarUrl getFormatPhotoUrl:82 hight:82]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"userDefaultIcon"]];
    ...
    _contentLb.frame = CGRectMake(_nameLb.minX, _portraitBtn.maxY - _nameLb.height_ + 1 , kScreenWidth - 78, size.height);
    ...
    [_feed.attaches enumerateObjectsUsingBlock:^(TXPBAttach *attach, NSUInteger idx, BOOL *stop) {
        ...
        if (count == 1) {
            ...
            if (fileType == TXPBAttachTypeVedio) {
                ...
                //图片文件
                int tmpWidth = (kScreenWidth - 78) * 3/4;
                UIButton *photoImgView = [UIButton buttonWithType:UIButtonTypeCustom];
                photoImgView.frame = CGRectMake(0, 0, tmpWidth, tmpWidth);
                photoImgView.backgroundColor = kColorCircleBg;
                photoImgView.imageView.contentMode = UIViewContentModeScaleAspectFill;
                [photoImgView TX_setImageWithURL:[NSURL URLWithString:[fileUrl getFormatPhotoUrl:tmpWidth hight:tmpWidth]] forState:UIControlStateNormal placeholderImage:nil completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
                    if (error) {
                ...
                photoImgView.backgroundColor = kColorCircleBg;
                photoImgView.imageView.contentMode = UIViewContentModeScaleAspectFill;
                [photoImgView TX_setImageWithURL:[NSURL URLWithString:[fileUrl getFormatPhotoUrl:width hight:height]] forState:UIControlStateNormal placeholderImage:nil completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
                    if (error) {
                        [photoImgView setBackgroundImage:[UIImage imageNamed:@"tp_320x240"] forState:UIControlStateNormal];
                    }else{
                        [photoImgView setBackgroundColor:[UIColor clearColor]];
            ...
            photoImgView.imageView.contentMode = UIViewContentModeScaleAspectFill;
            [photoImgView TX_setImageWithURL:[NSURL URLWithString:[fileUrl getFormatPhotoUrl:width hight:width]] forState:UIControlStateNormal placeholderImage:nil completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
                if (error) {
                    [photoImgView setBackgroundImage:[UIImage imageNamed:@"tp_148x148"] forState:UIControlStateNormal];
                }else{
                    [photoImgView setBackgroundColor:[UIColor clearColor]];
...
//    TXUser *feedUser = [[TXChatClient sharedInstance] getUserByUserId:feed.userId error:nil];
    if (user.userId == feed.userId || (user.userType == TXPBUserTypeTeacher && _feed.userType == TXPBUserTypeParent) || _feed.feedType == TXFeedTypeActivity) {
...
//        _lineView.frame = CGRectMake(0, _commentView.maxY + 13 - kLineHeight, kScreenWidth, kLineHeight);
//    }else{
//        _lineView.frame = CGRectMake(0, _moreView.maxY + 13 - (_moreView.height_ - timeSize.height)/2 - kLineHeight, kScreenWidth, kLineHeight);
//    }
    _lineView.frame = CGRectMake(0, 0, kScreenWidth, kLineHeight);
