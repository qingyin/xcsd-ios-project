...
- (void)viewDidLoad {
    ...
    _listTableView = [[UITableView alloc] initWithFrame:CGRectMake(0,navHeight,self.view.width_ ,self.view.height_ - navHeight) style:UITableViewStylePlain];
    ...
    //创建标题效果
//    [self setupFeedNavigationBarView];
    //获取最新亲子圈列表
    self.type = RequestType_Header;
...
- (void)setupChatToolBarView
{
    ...
    UINavigationController *rootViewController = (UINavigationController *)delegate.window.rootViewController;
    UIViewController *vc = rootViewController.topViewController;
    _msgInputView = [[TXMessageInputView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(rootViewController.view.frame), CGRectGetWidth(rootViewController.view.frame), kChatToolBarHeight)];
...
//    _navigationBarMaskView.alpha = 0.9;
    //滚动时的top图
...
//    _blurBarView.alpha = 0.f;
    ...
    CGFloat navHeight = IOS7_OR_LATER?kNavigationHeight + kStatusBarHeight:kNavigationHeight;
    // 左按钮
    self.btnLeft = [[CustomButton alloc] initWithFrame:CGRectMake(0, navHeight - kNavigationHeight, 100, kNavigationHeight)];
    self.btnLeft.showBackArrow = YES;
    self.btnLeft.tag = TopBarButtonLeft;
    ...
    self.btnLeft.titleLabel.font = kFontMiddle;
    self.btnLeft.titleEdgeInsets = UIEdgeInsetsMake(0, kEdgeInsetsLeft - 4, 0, 0);
    self.btnLeft.imageEdgeInsets = UIEdgeInsetsMake(0, kEdgeInsetsLeft, 0, 0);
    self.btnLeft.exclusiveTouch = YES;
    ...
    [self.btnLeft setTitleColor:kColorNavigationTitle forState:UIControlStateNormal];
    [self.btnLeft setTitleColor:kColorNavigationTitleDisable forState:UIControlStateDisabled];
    ...
    _addFeedButton.imageEdgeInsets = UIEdgeInsetsMake(0, 100 - 10 - 23, 0, 10);
...
//                [tmpObject reloadData];
            ...
            dispatch_group_notify(group, queue, ^{
                ...
                if (tmpObject.type == RequestType_Header) {
                    ...
                    //                    tmpObject.listArr = [NSMutableArray arrayWithArray:array1];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        return [CircleListHeaderCell GetHeaderCellHeight:_isShowNews];
    }
    else if (!_listArr.count && !_isFirstLoad){
        return _listTableView.height_ - [CircleListHeaderCell GetHeaderCellHeight:_isShowNews];
    }else{
//        TXFeed *feed = _listArr[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        cell.newsBgView.hidden = !_isShowNews;
        ...
        [cell setPortrait:user.avatarUrl andNickname:user.nickname];
        if (!_listArr.count && !_isFirstLoad){
            cell.backgroundColor = kColorWhite;
            cell.contentView.backgroundColor = kColorWhite;
        }else{
        ...
        if (!cell) {
            ...
            defaultBtn.frame = CGRectMake(0, 0, 156, 156);
            defaultBtn.center = CGPointMake(kScreenWidth/2, (_listTableView.height_ - [CircleListHeaderCell GetHeaderCellHeight:_isShowNews])/2);
...
//            [tmpArr addObject:@{@"name":userName,@"userId":userId}];
        }else{
    ...
    likeSize = [likeLabel sizeThatFits:CGSizeMake(kScreenWidth - 83 - 25, MAXFLOAT)];
...
+ (NSMutableArray *)getAttrobuteLabelArr:(TXFeed *)feed
{
    ...
    for (id tmpComment in feed.circleComments) {
        ...
        CGSize size = [commentLabel sizeThatFits:CGSizeMake(kScreenWidth - 86 - 18, MAXFLOAT)];
...
//    if (offsetY > 0) {
//        if (offsetY >= 100) {
//            if (_isNormalAddFeedImage) {
//                _isNormalAddFeedImage = NO;
//                [_addFeedButton setImage:[UIImage imageNamed:@"circle_new_1"] forState:UIControlStateNormal];
//            }
//            _statusBarWhite = NO;
//            [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
...
//            _barTitleLb.textColor = kColorNavigationTitle;
//        }else {
//            CGFloat offsetScale = offsetY / 200;
//            if (!_isNormalAddFeedImage) {
//                _isNormalAddFeedImage = YES;
//                [_addFeedButton setImage:[UIImage imageNamed:@"circle_new"] forState:UIControlStateNormal];
//            }
//            _statusBarWhite = YES;
//            if (IOS7_OR_LATER) {
//                [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
...
//                [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleBlackOpaque];
//            }
//            _navigationBarMaskView.alpha = 0.9f - (offsetScale > 0.9f ? 0.9f : offsetScale);
//            CGFloat textDiff = 211;  //255 - 44
//            _barTitleLb.textColor = RGBCOLOR(255 - textDiff * offsetScale, 255 - textDiff * offsetScale, 255 - textDiff * offsetScale);
//        }
//        if (offsetY >= 200) {
//            _blurBarView.alpha = 1.f;
//        } else {
////            _navigationBarMaskView.alpha = 1 - (offsetY / NAVBAR_CHANGE_POINT);
//            CGFloat offsetScale = offsetY / 200;
//            _blurBarView.alpha = offsetScale;
//        }
//    }else{
//        _navigationBarMaskView.alpha = 0.9f;
//        _blurBarView.alpha = 0.f;
