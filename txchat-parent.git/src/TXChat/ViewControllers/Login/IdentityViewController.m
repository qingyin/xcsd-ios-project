...
//    __weak typeof(self)tmpObject = self;
    ...
    _birthdayDetailLb.text = [self handleDate:[NSDate dateWithTimeIntervalSince1970:self.child.birthday / 1000]];
    ...
    [birthdayBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
        ...
        NSDate *defaultDate = [NSDate dateWithTimeIntervalSince1970:([NSDate date].timeIntervalSince1970 - minDate.timeIntervalSince1970) / 2 + minDate.timeIntervalSince1970];
        [self showDatePickerWithCurrentDate:[NSDate date] minimumDate:minDate maximumDate:[NSDate date] selectedDate:defaultDate selectedBlock:^(NSDate *selectedDate) {
//            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
//            [dateFormatter setDateFormat:@"yyyy-MM-dd"];
            if (![selectedDate isEqualToDate:defaultDate]) {
                tmpObject.birthdayDetailLb.text = [tmpObject handleDate:selectedDate];
            }else{
                
                tmpObject.birthdayDetailLb.text = [tmpObject handleDate:defaultDate];
            }
        }];
    }];
    ...
    _identilyDetailLb.text = [self handleParentType:_txUser];
    ...
    _nameDetailLb = [[UILabel alloc] initWithFrame:CGRectMake(nameLb.maxX+kEdgeInsetsLeft, nameLb.minY, kScreenWidth - (nameLb.maxX+kEdgeInsetsLeft+kEdgeInsetsLeft+25), nameLb.height_)];
    _nameDetailLb.textAlignment = NSTextAlignmentRight;
    _nameDetailLb.font = kFontMiddle;
    _nameDetailLb.textColor = kColorGray;
    _nameDetailLb.text = self.child.guarder;
    ...
    [nameBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
        ...
        if (![tmpObject.nameDetailLb.text isEqualToString:KGUARDERDEFAULT]) {
...
- (void)onClickBtn:(UIButton *)sender{
    if (sender.tag == TopBarButtonLeft) {
        ...
        }else if(!_nameDetailLb.text.length || [_nameDetailLb.text isEqualToString:KGUARDERDEFAULT]){
            [self showFailedHudWithTitle:@"请输入监护人姓名"];
...
// 处理孩子家长类型
- (NSString *)handleParentType:(TXUser *)user{
    
    
    for (NSInteger i = 0; i < user.childUserIdAndRelationsList.count; ++i) {
        
        TXPBChild *child = user.childUserIdAndRelationsList[i];
        
        if (child.userId == self.child.userId) {
            
            return [NSString getParentTypeStr:child.parentType];
        }
    }
    return @"";
}
// 处理日期
- (NSString *)handleDate:(NSDate *)date{
    ...
    return [birthday isEqualToString:@"1970-01-01"] ? @"" : birthday;
}
