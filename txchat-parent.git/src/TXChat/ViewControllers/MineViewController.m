...
- (void)viewDidLoad {
    ...
    {
        BOOL isExist = NO;
        NSString *homeMenuStr = [userProfiles objectForKey:KHOMELIST];
        if(homeMenuStr != nil) {
            NSArray *homeMenuList = [homeMenuStr componentsSeparatedByString:@","];
            if (homeMenuList && [homeMenuList containsObject:TX_PROFILE_KEY_OPTION_INSURANCE]) {
                isExist = YES;
            }
        }
        if (isExist) {
            cateArray = @[@{@"title":@"邀请家人",@"img":@"mine_invitation",@"type":@(MineviewListType_Invitation)},
//                          @{@"title":@"云卫士卡号",@"img":@"mine_guardian",@"type":@(MineviewListType_Guardian)},
                          @{@"title":@"孩子保险",@"img":@"mine_safe",@"type":@(MineviewListType_Safe)}];
        }else{
            cateArray = @[@{@"title":@"邀请家人",@"img":@"mine_invitation",@"type":@(MineviewListType_Invitation)},
//                          @{@"title":@"云卫士卡号",@"img":@"mine_guardian",@"type":@(MineviewListType_Guardian)}
                          ];
...
//                      @{@"title":@"云卫士卡号",@"img":@"mine_guardian",@"type":@(MineviewListType_Guardian)}
    ...
    _listArr = @[
                 @[@{@"type":@(MineviewListType_Info)}],//顶部信息
                 cateArray,
//                 @[
//  @{@"title":@"我的关注",@"img":@"mine_follow",@"type":@(MineviewListType_Follow)}],
                 @[
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
     ...
     //bay zhnag
        nameLb.text = user.nickname.length>0?user.nickname:user.combinedNickName;
        if(nameLb.text.length <= 0){
...
#endif
        twoDimensionCodeView.hidden = YES;
        return cell;
    }else{
