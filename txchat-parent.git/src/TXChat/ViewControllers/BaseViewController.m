...
//    _customNavigationView.clipsToBounds = YES;
    ...
    [_userLb setFont:kFontSuper_b];
    _titleLb.textColor=kColorNavigationTitle;
    [_customNavigationView addSubview:_userLb];
    [_userLb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(_customNavigationView.centerY/2).mas_offset(25);
        make.left.mas_equalTo(_customNavigationView.centerX).mas_offset(100);
        make.size.mas_equalTo(CGSizeMake(160, 30));
    }];
    
   
    
    _selectUserBtn=[[CustomButton alloc]init];
    _selectUserBtn.backgroundColor=[UIColor clearColor];
    [_customNavigationView addSubview:_selectUserBtn];
    [_selectUserBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(_userLb.mas_top).mas_offset(10);
        make.left.mas_equalTo(_userLb.mas_right);
                           ...
                           microphoneGranted:(BOOL)isGrantMacrophone
{
    ...
    if (!isGrantCamera && !isGrantMacrophone) {
        ...
        alertTitle = @"没有权限访问您的麦克风,请到“设置-隐私-麦克风”里把乐学堂”的开关打开即可";
...
//显示通讯录权限弹窗
- (void)showAddressPermissionDeniedAlert
{
    ...
    NSString *alertTitle = @"没有权限访问您的通讯录,请到“设置-隐私-通讯录”里把“乐学堂”的开关打开";
    [self showAlertViewWithMessage:alertTitle andButtonItems:setItem, nil];
}
               ...
               showSuccessImage:(BOOL)showSuccessImage
{
    ...
    MBProgressHUD *finishHud = [[MBProgressHUD alloc] initWithView:self.view];
     finishHud.layer.cornerRadius = 1.f;
              ...
              showSuccessImage:(BOOL)showSuccessImage
{
  ...
  //  failedHud.layer.cornerRadius = 1.f;
