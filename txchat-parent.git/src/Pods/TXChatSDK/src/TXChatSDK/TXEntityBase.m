...
- (TXEntityBase *)init {
    if (self = [super init]) {
        ...
        [_transientProperties addObject:@"childUserIdAndRelationsList"];
    }
    return self;
}
...
- (NSString *)generateReplaceIntoSql {
    ...
    while (subclass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyCount; i++) {
            ...
            if ([[self transientProperties] containsObject:@(propertyName)] || strcmp(propertyName, "id") == 0 || strcmp(propertyName, "childUserIdAndRelationsList")==0) {
                continue;
            }
    ...
    while (subclass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyCount; i++) {
            ...
            if ([[self transientProperties] containsObject:@(propertyName)] || strcmp(propertyName, "id") == 0
                || strcmp(propertyName, "childUserIdAndRelationsList") == 0) {
                continue;
            }
...
- (instancetype)loadValueFromFMResultSetInner:(FMResultSet *)resultSet {
    ...
    while (subclass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyCount; i++) {
            ...
            } else if(strcmp(propertyType, "@\"NSArray\"") == 0){
                NSLog(@"==================================================");
                NSLog(@"message:Unexpected property %s %s", propertyName, propertyType);
                NSLog(@"==================================================");
            }
            else {
                NSLog(@"==================================================");
