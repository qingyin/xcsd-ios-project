...
{
	NSDictionary *dict = self.applicationManager.currentUserProfiles;
	if (dict) {
		return [dict valueForKey:TX_PROFILE_KEY_CURRENT_TOKEN] ? dict[TX_PROFILE_KEY_CURRENT_TOKEN] : @"";
	}
	return nil;
}
...
- (void)fetchHomeWorks:(BOOL)isInbox maxHomeWorkId:(int64_t)maxHomeWorkId onCompleted:(void (^)(NSError *error, NSArray/*<XCSDHomeWork>*/ *xcsdHomeWork, BOOL isInbox, BOOL hasMore, BOOL lastOneHasChanged))onCompleted {
    [self.xcsdHomeWorkManager fetchHomeWorks:YES maxHomeWorkId:maxHomeWorkId onCompleted:onCompleted];
}
- (NSArray *)getHomeWork:(int64_t)maxhomeWorkId count:(int64_t)count error:(NSError **)outError {
    return [self.xcsdHomeWorkManager queryHomeWork:maxhomeWorkId count:count error:outError];
}
- (XCSDHomeWork *)getLastHomework:(NSError **)outError{
    return [self.xcsdHomeWorkManager queryLastHomework:outError];
}
...
- (void)DeletehomeworId:(int64_t)homeworkId
            onCompleted:(void (^)(NSError *error))onCompleted{
    [self.xcsdHomeWorkManager DeletehomeworId:homeworkId onCompleted:onCompleted];
}

- (void)ReadhomeworkId:(int64_t)homeworkId
                 onCompleted:(void (^)(NSError *error))onCompleted{
    [self.xcsdHomeWorkManager ReadhomeworkId:homeworkId onCompleted:onCompleted];
}
- (void)RankHomeWorksChildUserId:(int64_t)ChildUserId
    ...
    [self.xcsdHomeWorkManager RankHomeWorksChildUserId:ChildUserId onCompleted:onCompleted];
}
- (void)fetchChildAttendance:(int64_t)ChildUserId
    ...
    [self.xcsdHomeWorkManager fetchChildAttendance:ChildUserId onCompleted:onCompleted];
}
...
- (NSArray *)getCourses:(NSInteger) courseId count:(NSInteger)count{
    return [self.courseManager queryCourses:courseId count:count];
}

- (void)fetchCourseList:(NSInteger)aPage
            onCompleted:(void (^)(NSError *error, NSArray *lessons,BOOL hasMore))onCompleted{
    [self.courseManager fetchCourseList:aPage onCompleted:onCompleted];
}
...
- (void)HomeworkResult:(NSInteger) childId onCompleted:(void(^)(NSError *error, XCSDPBAbilityStatResponse *abilityDetails)) onCompleted{
    [self.abilityManager fetchHomeworkResult:childId onCompleted:^(NSError *error, XCSDPBAbilityStatResponse *abilityDetails) {
        onCompleted(error, abilityDetails);
    }];
}
/**
 *  获取游戏状态
...
- (void)GameStatus:(NSInteger) userId ability:(XCSDPBAbility) ability onCompleted:(void(^)(NSError *error, NSInteger totalScore, NSArray *gameList)) onCompleted{
    ...
    [self.abilityManager fetchGameStatus:userId ability:ability onCompleted:^(NSError *error, NSInteger totalScore, NSArray *gameList) {
        onCompleted(error, totalScore, gameList);
    }];
}
@end