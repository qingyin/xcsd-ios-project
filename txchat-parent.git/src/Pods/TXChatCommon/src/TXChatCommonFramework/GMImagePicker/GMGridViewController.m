...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([self.picker.selectedAssets count] >= (self.picker.maxSelectionNumber - self.picker.currentSelectedCount)) {
        if (self.picker.delegate && [self.picker.delegate respondsToSelector:@selector(assetsPickerControllerDidReachMaxSelection:)]) {
            [self.picker.delegate assetsPickerControllerDidReachMaxSelection:self.picker];
        }
        return NO;
    }
    else
