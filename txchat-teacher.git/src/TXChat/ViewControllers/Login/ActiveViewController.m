...
- (void)initView{
    ...
    _phoneTextField.placeholder = _isPwd?@"请输入已注册的手机号":((_password && _password.length)?@"输入手机号":@"输入预留手机号");
    ...
    _codeTipLb.textColor = ColorNavigationTitle;
    ...
    _codeBtn.layer.borderColor = ColorNavigationTitle.CGColor;
    ...
    [voiceBtn setTitleColor:ColorNavigationTitle forState:UIControlStateNormal];
//    [voiceBtn setTitle:@"语音验证码" forState:UIControlStateNormal];
    [voiceBtn setTitle:@"联系我们" forState:UIControlStateNormal];
...
//        [tmpObject onGetCodeBtn:YES];
        [self.navigationController pushViewController:[[PublishmentDetailViewController alloc] initWithLinkURLString:KURL_FEEDBACK] animated:YES];
    }];
    [voiceBtn sizeToFit];
    ...
    [agreementBtn setTitleColor:ColorNavigationTitle forState:UIControlStateNormal];
...
- (void)recoveryVerificationCode
{
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:numStr];
    [attributedString addAttribute:NSForegroundColorAttributeName value:ColorNavigationTitle range:NSMakeRange(0, numStr.length)];
    _codeTipLb.attributedText = attributedString;
    _codeBtn.layer.borderColor = ColorNavigationTitle.CGColor;
}
        ...
        [[TXChatClient sharedInstance] changeMobilePhoneNumber:_phoneTextField.text verifyCode:_codeTextField.text onCompleted:^(NSError *error) {
            ...
            if (error) {
                ...
                [IdentityViewController loginEaseMob:[NSString stringWithFormat
                                                      :@"%@",@(user.userId)] andPwd:pwd];
                //保存到UserDefaults
                [[NSUserDefaults standardUserDefaults] setValue:[NSString stringWithFormat:@"%@",@(user.userId)] forKey:kEaseMobUserName];
                [[NSUserDefaults standardUserDefaults] setValue:phone forKey:kLocalUserName];
