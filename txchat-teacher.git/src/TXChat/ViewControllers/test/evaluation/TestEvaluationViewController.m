...
- (void)viewDidLoad
{
    ...
    [_contentView setBorderWithWidth:0.5 andCornerRadius:12 andBorderColor:RGBCOLOR(160, 160, 160)];
...
-(void)getEvalutaion
{
    ...
    NSString *userId = [NSString stringWithFormat:@"%lld", [TXApplicationManager sharedInstance].currentUser.userId];
	[EduApi getEvaluationWithToken:[UserPreference getToken]  testID:self.testInfo.id childID:userId completion:^BOOL(BOOL success, id response) {
...
-(void)getEvaluationSuccess:(BaseResponse *)result
{
    ...
    TXUser *teacherUser = [TXApplicationManager sharedInstance].currentUser;
	
	//宝宝信息
	[_childImageView sd_setImageWithURL:[NSURL URLWithString:teacherUser.avatarUrl] placeholderImage:[UIImage imageNamed:@"head_default.png"]];
	...
	_rankDescLabel.text = [NSString stringWithFormat:@"%@是第%ld名参加此测试的用户喔～",teacherUser.nickname,_evaluationInfo.testSerialNumber];
...
//	CGFloat height = [LabelUtils heightForLabel:_label WithText:_label.text andMinHeight:20];
    CGFloat height = [UILabel heightForLabelWithText:_label.text maxWidth:_label.width_ font:_label.font];
