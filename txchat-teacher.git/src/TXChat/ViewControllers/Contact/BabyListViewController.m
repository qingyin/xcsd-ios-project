...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TParentTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"TParentTableViewCell" owner:self options:nil] objectAtIndex:0];
    ...
    [cell.userNameLabel setText: KCONVERTSTRVALUE(parentUser.nickname)];
    [cell.userImageView TX_setImageWithURL:[NSURL URLWithString:[parentUser getFormatAvatarUrl:40.0f hight:40.0f]] placeholderImage:[UIImage imageNamed:@"userDefaultIcon"]];
    NSInteger count = [(NSArray *)[_babyList objectAtIndex:indexPath.section] count];
    if(indexPath.row == count - 2)
    {
        [cell.seperatorLine setHidden:YES];
    }
    [cell.inActiveBtn setHidden:parentUser.activated];
    if(!parentUser.activated)
    {
        @weakify(self);
        [cell.inActiveBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            @strongify(self);
            [TXProgressHUD showHUDAddedTo:self.view withMessage:@""];
    ...
    cell.tag = KPARENTCELLBASETAG+indexPath.section;
    NSString *mobile = parentUser.mobilePhoneNumber;
    __weak __typeof(&*self) weakSelf=self;  //by sck
    cell.callBlock = ^(NSInteger viewTag)
    {
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView
{
    ...
    for(NSArray *index in array)
    {
        if([index count] > 0)
        {
            ...
            {
                [sectionIndex addObject:[NSString stringWithFormat:@"%@",[user.nicknameFirstLetter substringToIndex:1]]];
            }
        }
    }
    return sectionIndex;
