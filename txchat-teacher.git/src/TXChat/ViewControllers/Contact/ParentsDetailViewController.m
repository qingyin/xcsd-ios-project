...
- (void)viewDidLoad
{
    ...
    if(currentUser.userId != _user.userId)
    {
        ...
        if(_user.activated)
        {
            ...
            [callBtn setBackgroundColor:KColorAppMain];
            [callBtn setImage:[UIImage imageNamed:@"callIcon"] forState:UIControlStateNormal];
            [callBtn setTitleColor:kColorGray forState:UIControlStateHighlighted];
            [callBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 5)];
            [callBtn setTitleEdgeInsets:UIEdgeInsetsMake(0, 15, 0, 0)];
            [callBtn setTitleColor:kColorWhite forState:UIControlStateNormal];
            ...
            [sendMessageBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.left.mas_equalTo(callBtn.mas_right).with.offset(0.5);
                make.right.mas_equalTo(superview.mas_right).with.offset(0);
                make.height.mas_equalTo(btnHight);
                make.width.mas_equalTo(callBtn);
            }];
            [callBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.mas_equalTo(sendMessageBtn.mas_top);
                make.left.mas_equalTo(superview.mas_left);
                make.right.mas_equalTo(sendMessageBtn.mas_left).with.offset(-0.5);
                make.height.mas_equalTo(btnHight);
                make.width.mas_equalTo(sendMessageBtn);
            }];
        }
        else
        {
...
-(void)createTitles
{
    if(_isParent)
    {
        ...
        _titleArray = @[@[ @{KNAMEKEY:@"手机", KTYPEKEY:@(ParentDetailType_Mobile)},@{KNAMEKEY:@"宝宝性别", KTYPEKEY:@(ParentDetailType_Sex)}, @{KNAMEKEY:@"班级", KTYPEKEY:@(ParentDetailType_Class)}, @{KNAMEKEY:@"学校", KTYPEKEY:@(ParentDetailType_Garden)}]];
    }
    else
    {
        _titleArray = @[@[ @{KNAMEKEY:@"手机", KTYPEKEY:@(ParentDetailType_Mobile)}, @{KNAMEKEY:@"职位", KTYPEKEY:@(ParentDetailType_Position)}, @{KNAMEKEY:@"学校", KTYPEKEY:@(ParentDetailType_Garden)}]];
    }
}
-(void)createUser
{
    ...
    if(indexPath.section == 0)
    {
        ...
        if(_isParent)
        {
            ...
            switch (type.integerValue) {
                case ParentDetailType_Mobile:
                    content = _user.mobilePhoneNumber;
                    break;
...
-(void)call:(id)sender
{
    if(_user.mobilePhoneNumber != nil && [_user.mobilePhoneNumber length] > 0)
    {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"telprompt://%@",_user.mobilePhoneNumber]]];
    }
    else
    {
