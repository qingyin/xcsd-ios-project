...
- (void)setupCellWithThumbnailName:(NSString *)thumbnailName
{
    ...
    NSString *imgStr = [thumbnailName getFormatPhotoUrl:width hight:width];
    __weak UIImageView *tmpObject = _thumbnailImgView;
    [_thumbnailImgView TX_setImageWithURL:[NSURL URLWithString:imgStr] placeholderImage:nil completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
        if (error) {
