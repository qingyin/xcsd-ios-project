...
static NSString *const kFoundRuleExplainUrlString = @"http://h5.tx2010.com/shop.do?ruleExplain";
...
- (void)viewDidLoad {
    ...
    if (_foundType == FoundType_Event) {
        ...
        //积分商城
        [self.btnRight setTitle:@"规则说明" forState:UIControlStateNormal];
    }else{
...
- (void)createCustomNavBar
{
    ...
    if (_foundType == FoundType_Shop || _foundType == FoundType_Event) {
        UIButton *closeBtn = [[UIButton alloc] initWithFrame:CGRectMake(60*kScale1, self.customNavigationView.height_ - kNavigationHeight, 40, kNavigationHeight)];
...
- (void)onClickBtn:(UIButton *)sender
{
    if (sender.tag == TopBarButtonLeft) {
        ...
        if (_foundType == FoundType_Event) {
            ...
            //规则说明
            FoundWebViewController *vc = [[FoundWebViewController alloc] init];
            vc.foundType = FoundType_RuleExplain;
            vc.enterVc = _enterVc;
            [self.navigationController pushViewController:vc animated:YES];
        }else{
            [self.navigationController popViewControllerAnimated:YES];
...
- (void)loadFoundWebContentData
{
    ...
    }else if (_foundType == FoundType_RuleExplain) {
        //规则说明
        if ([TXSystemManager sharedManager].isDevVersion) {
            NSString *baseUrl = [[TXSystemManager sharedManager] webBaseUrlString];
            requestUrl = [NSString stringWithFormat:@"%@shop.do?ruleExplain",baseUrl];
            //            //添加token
            //            if (self.userToken && [self.userToken length]) {
            //                requestUrl = [NSString stringWithFormat:@"%@?token=%@",requestUrl,self.userToken];
            //            }
        }else{
            requestUrl = kFoundRuleExplainUrlString;
        }
    }else{
