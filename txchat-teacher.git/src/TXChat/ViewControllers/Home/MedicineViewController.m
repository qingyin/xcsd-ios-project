...
- (void)viewDidLoad
{
    ...
    [self addEmptyDataImage:NO showMessage:@"没有喂药信息"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TMedicineTableViewCell *medicineCell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (medicineCell == nil) {
        medicineCell = [[[NSBundle mainBundle] loadNibNamed:@"TMedicineTableViewCell" owner:self options:nil] objectAtIndex:0];
    }
    TXFeedMedicineTask *medicine = [_medicineList objectAtIndex:indexPath.row];
    [medicineCell.fromLabel setText:medicine.parentUsername];
    [medicineCell.headerImageview TX_setImageWithURL:[NSURL URLWithString:[medicine.parentAvatarUrl getFormatPhotoUrl:40 hight:40]] placeholderImage:[UIImage imageNamed:@"userDefaultIcon"]];
    [medicineCell.contentLabel setText:medicine.content];
    [medicineCell.timeLabel setText:[NSDate timeForChatListStyle:[NSString stringWithFormat:@"%@", @(medicine.createdOn/1000)]]];
