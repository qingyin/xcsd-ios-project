//
//  PublishmentDetailViewController.m
//  TXChat
//
...
@interface PublishmentDetailViewController ()
<UIWebViewDelegate,
UMSocialUIDelegate,
...
@implementation PublishmentDetailViewController
...
- (void)shareLinkToSocial
{
    ...
    snsPlatform.snsClickHandler = ^(UIViewController *presentingController, UMSocialControllerService * socialControllerService, BOOL isPresentInController){
        PublishmentDetailViewController *detailVc = (PublishmentDetailViewController *)presentingController;
        if (detailVc) {
    ...
    innerPlatform.snsClickHandler = ^(UIViewController *vc, UMSocialControllerService *controllerService, BOOL isPresentInController){
        ...
        PublishmentDetailViewController *detailVC = (PublishmentDetailViewController *)vc;
        
        if (detailVC) {
            
            ShareSelectController *shareVC = [[ShareSelectController alloc] init];
            shareVC.url = detailVC.linkUrlString;
            shareVC.articleTitle = detailVC.articleTitle;
            shareVC.coverImageUrl = detailVC.coverImageUrl;
    ...
    //设置你要在分享面板中出现的平台
    [UMSocialConfig setSnsPlatformNames:@[UMShareToWechatTimeline,UMShareToWechatSession,UMShareToQQ,@"CopyLink", @"innerShare"]];
    //分享
