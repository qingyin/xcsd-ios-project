...
- (NSString *)formattedDateDescription
{
    ...
    if (timeInterval < 60) {
        ...
        return [NSString stringWithFormat:@"%.f天前", (float)timeInterval / 3600];
...
{
    double dateTimeStamp = [timeStamp doubleValue];
    double nowTimeStamp = [[NSDate date] timeIntervalSince1970];
    double timeInterval = nowTimeStamp - dateTimeStamp;
    NSInteger intervalHour = timeInterval / D_HOUR;
    NSInteger dayHour = [[NSDate date] hour];
    NSInteger hour = dayHour - intervalHour;
    NSString *ret = @"";
    NSDateFormatter *dateFormatter = nil;
    //hasAMPM==TURE为12小时制，否则为24小时制
    NSString *formatStringForHours = [NSDateFormatter dateFormatFromTemplate:@"j" options:0 locale:[NSLocale currentLocale]];
    ...
    ret = [dateFormatter stringFromDate:[NSDate dateWithTimeIntervalSince1970:dateTimeStamp]];
    return ret;
}
...
+ (NSString *)timeForCircleStyle:(NSString *)timeStamp{
    ...
    NSTimeInterval  timeInterval =current - [timeStamp doubleValue];
    long temp = 0;
    NSString *result;
    if (timeInterval < 60) {
...
//刷卡时间格式
+ (NSString*)timeForCheckInStyle:(NSString *)timeStamp
{
    if (!timeStamp || ![timeStamp length]) {
        return nil;
    }
...
+(NSDate *)getDateThreeYearsAgo
{
    ...
    NSString *dateStr = [NSString stringWithFormat:@"%04ld-01-01", year-3];
