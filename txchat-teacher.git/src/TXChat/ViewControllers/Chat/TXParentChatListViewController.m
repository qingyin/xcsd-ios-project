...
- (void)viewDidLoad {
    ...
    _isHasCommunionData = [[[TXSystemManager sharedManager] chatListDataForKey:kChatListHasCommunionFlag] boolValue];
    //教师端添加联系人入口
...
//        [self.btnRight setImage:[UIImage imageNamed:@"qrCodeNav"] forState:UIControlStateNormal];
//        [self.btnRight addTarget:self action:@selector(onClickQRCodeButton:) forControlEvents:UIControlEventTouchUpInside];
    }
    ...
    //订阅请假消息事件
    [self subscribeCountType:TXClientCountType_Rest refreshBlock:^(NSInteger oldValue, NSInteger newValue, TXClientCountType type) {
    ...
    //注册教师成长红点事件
    [self subscribeCountType:TXClientCountType_JSB refreshBlock:^(NSInteger oldValue, NSInteger newValue, TXClientCountType type) {
        if (oldValue != newValue) {
            if (newValue > 0) {
                //更新时间和数据
                STRONGSELF
                NSTimeInterval currentTime = [[NSDate date] timeIntervalSince1970];
                [[TXSystemManager sharedManager] saveChatListData:@(currentTime) forKey:kChatListLastCommunionTime];
                [[TXSystemManager sharedManager] saveChatListData:@(YES) forKey:kChatListHasCommunionFlag];
                if (strongSelf) {
                    [strongSelf refreshCommunionDataSource];
                }
            }else{
                //用户已看过,红点应该消失，此时刷新界面
                STRONGSELF
...
- (void)onClickTopBtn:(UIButton *)sender{
    //联系人按钮
    ContactListViewController *classList = [[ContactListViewController alloc] init];
    [self.navigationController pushViewController:classList animated:YES];
}
//扫一扫
//- (void)onClickQRCodeButton:(UIButton *)sender
//{
////    TestViewController *testVc = [TestViewController new];
////    testVc.popupItem.title = @"周杰伦";
////    testVc.popupItem.subtitle = @"爱在西元前";
////    testVc.popupItem.progress = (float) arc4random() / UINT32_MAX;
////    [self.rdv_tabBarController presentPopupBarWithContentViewController:testVc animated:YES completion:nil];
////    return;
//    [[TXSystemManager sharedManager] requestCameraPermissionWithBlock:^(BOOL cameraGranted) {
//        TXAsyncRunInMain(^{
//            if (cameraGranted) {
//                //已授权
//                ReaderCodeViewController *readVc = [[ReaderCodeViewController alloc] init];
...
- (TXChatAttendanceConversation *)lastAttendanceConversation
{
    ...
    NSString *lastMsg = @"有一条新的请假消息";
    NSDictionary *restDict = [self countValueForType:TXClientCountType_Rest];
...
//获取教师成长消息
- (TXChatJSBConversation *)lastCommunionConversation
{
    BOOL isHasNewCommunion = NO;
    NSString *lastMsg = @"你有一条新的教师社区动态";
    NSDictionary *jsbDict = [self countValueForType:TXClientCountType_JSB];
    NSInteger jsbNewValue = [[jsbDict valueForKey:TXClientCountNewValueKey] integerValue];
    if (jsbNewValue > 0) {
        //有新的教师动态
        isHasNewCommunion = YES;
//        lastMsg = @"你有一条新的教师成长动态";
    }
    NSNumber *currentTime = [[TXSystemManager sharedManager] chatListDataForKey:kChatListLastCommunionTime];
    NSDictionary *attendanceDict = @{@"lastMsg": lastMsg,@"time": [NSString stringWithFormat:@"%@", currentTime], @"unreadNumbe":isHasNewCommunion ? @1 : @0};
    TXChatJSBConversation *conversation = [[TXChatJSBConversation alloc] initWithConversationAttributes:attendanceDict];
    return conversation;
}
...
//        }
        ...
        //添加教师帮
        if (_isHasCommunionData) {
            TXChatJSBConversation *attConv = [self lastCommunionConversation];
            if (attConv) {
                [sortArray addObject:attConv];
            }
        }
...
//刷新教师成长数据
- (void)refreshCommunionDataSource
{
    //允许添加考勤到列表中
    NSNumber *flagNumber = [[TXSystemManager sharedManager] chatListDataForKey:kChatListHasCommunionFlag];
    if (!flagNumber) {
        _isHasCommunionData = NO;
    }else {
        _isHasCommunionData = [flagNumber boolValue];
    }
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        BOOL isReturn = [self fetchUserConversationsList];
        if (isReturn) {
...
- (BOOL)canEditChatConversationsRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [_conversationList count]) {
        ...
        if ([item isKindOfClass:[TXChatWXYConversation class]]) {
            ...
            return YES;
        }else if ([item isKindOfClass:[TXChatJSBConversation class]]) {
            //教师帮
            return NO;
        }
        return YES;
...
- (void)didSelectChatConversationAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [_conversationList count]) {
        ...
        if ([item isKindOfClass:[TXChatEaseMobConversation class]]) {
            ...
            //通知cell
            TeacherNoticeListViewController *parentNotifyList = [[TeacherNoticeListViewController alloc] init];
            ...
            //请假列表
            LeavesListViewController *vc = [[LeavesListViewController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
        }else if ([item isKindOfClass:[TXChatJSBConversation class]]) {
            mobLogString = @"点击消息列表页的教师帮消息";
            THCommunionMessagesViewController *vc = [[THCommunionMessagesViewController alloc] init];
            vc.isMessage = YES;
            vc.bid = TX_PROFILE_KEY_OPTION_TEACHERCOMMUNITY;
            [self.navigationController pushViewController:vc animated:YES];
        }
        //友盟统计
