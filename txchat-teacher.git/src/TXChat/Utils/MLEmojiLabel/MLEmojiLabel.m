...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
	    NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"emotion.plist"];
...
//    [self initPlist];
    ...
    UIColor *commonLinkColor = kColorGray1;
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    for (NSTextCheckingResult *result in emojis) {
		...
		NSString *imageName = emojiDict[@"emotionName"][emojiKey];
        if (!self.customEmojiRegularExpression) {
...
{
    if (self.emojiDelegate && [self.emojiDelegate respondsToSelector:@selector(attributedLabel:didSelectLinkWithTextCheckingResult:)]) {
        [self.emojiDelegate attributedLabel:self didSelectLinkWithTextCheckingResult:result];
    }
