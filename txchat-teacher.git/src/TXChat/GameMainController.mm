...
+(void)callNativeGameTestReport:(NSString *)testId
{
	...
	[[TXChatClient sharedInstance].dataReportManager reportEventBid:XCSDPBEventTypeGameTest bid:testId];
}
	...
	[[TXHttpClient sharedInstance] sendRequest:url
								   ...
								   completed:{
									   ...
									   dispatch_async(dispatch_get_main_queue(), ^{
										   ...
										   NSLog(@"%lu bytes string is '%s'",[responseData length],[responseData bytes]);
										   
										   DDLogDebug(@"callNativeHttpPost->innerError:%@,%ld",innerError,(long)innerError.code);
										   
										   if (innerError) {
											   instace->app_delegate->executeJSFunctionWithParam("{\"result\":400}","httpPostCallBack");
										   }else{
											   ...
											   instace->app_delegate->executeJSFunctionWithParam(str,"httpPostCallBack");
										   }
									   });
								   }
							}];
...
//    cocos2d::EventListenerCustom *_listener;
    ...
    app_delegate = (CCAppDelegate*) app;
    app_delegate->setupMainScene();
	
	app_delegate->executeJSFunctionWithParam([self.param cStringUsingEncoding:NSASCIIStringEncoding],"EnterCocos");
    instace = self;
