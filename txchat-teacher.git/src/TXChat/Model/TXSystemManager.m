...
NSString * const kChatListLastCommunionTime = @"chatListCommunionTime";
NSString * const kChatListHasCommunionFlag = @"chatListHasCommunionFlag";
...
-(void)initTXChatSDK
{
    NSString *sdkVersion = [NSString stringWithFormat:@"%@_%@", TX_CHAT_CLIENT_PLATFORM,  [[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleShortVersionString"]];
    [[TXChatClient sharedInstance] setupWithVersion:sdkVersion];
}
//配置服务器信息
- (void)setupServerModeInfo
{
    ...
    //正式环境 modify by sck
    [_serverModeDict setObject:@{kTXServerHost:@"service.xcsdedu.com",kTXServerPort:@"80",kTXEaseMobAppKey:KHuanXin_AppKey_Dis,
                                 kTXWebBaseUrl:@"http://service.xcsdedu.com",kJSHostUrl:KURL_H5_SERVER_ADDRESS_DIS} forKey:@"publicFormal"];


	//测试环境
	[_serverModeDict setObject:@{kTXServerHost:@"121.41.101.14",kTXServerPort:@"8080",kTXEaseMobAppKey:KHuanXin_AppKey_Test,kTXWebBaseUrl:@"http://121.41.101.14:8080/",kJSHostUrl:KURL_H5_SERVER_ADDRESS_TEST} forKey:@"privateTest"];
	
	//开发环境
	[_serverModeDict setObject:@{kTXServerHost:@"121.40.16.212",kTXServerPort:@"8080",kTXEaseMobAppKey:KHuanXin_AppKey_Dev,kTXWebBaseUrl:@"http://121.40.16.212:8080/",kJSHostUrl:KURL_H5_SERVER_ADDRESS_DEV} forKey:@"privateDev"];
...
//        __weak typeof(AppDelegate *)tmpObject = appdelegate;
...
- (void)checkIsAllowPlayMediaByCurrentNetworkStatus:(void (^)(BOOL isReachable, BOOL isCanPlay)) block{
    ...
    switch (status) {
        ...
        case ReachableViaWWAN:
            if (self.mediaNetworkType == TXMediaPlayNetworkType_OnlyByWifi) {
                isCanPlay = NO;
            }
        default:
            break;
...
#pragma mark - 刷卡语音播报功能
- (void)setCheckInVoiceSpeaking:(BOOL)checkInVoiceSpeaking
{
    if (_checkInVoiceSpeaking == checkInVoiceSpeaking) {
        return;
    }
    _checkInVoiceSpeaking = checkInVoiceSpeaking;
//    //测试语音
//    if (_checkInVoiceSpeaking) {
//        //开启语音
//        _voiceTimer = [NSTimer timerWithTimeInterval:5.f target:self selector:@selector(fetchNewCheckInVoiceAndSpeech) userInfo:nil repeats:YES];
...
//获取新的语音并刷卡
- (void)fetchNewCheckInVoiceAndSpeech
{
    NSString *name = [NSString stringWithFormat:@"涛涛%@刷卡了",@(arc4random() % 10 + 1)];
