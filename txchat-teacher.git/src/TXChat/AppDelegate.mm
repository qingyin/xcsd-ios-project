...
//	
//	XCSDDataReportManager* dm = [[XCSDDataReportManager alloc] init];
//	[[NSNotificationCenter defaultCenter] postNotificationName:KDataReport object:nil userInfo: [NSDictionary dictionaryWithObject:@"event" forKey:@""]];
...
//-(void)initMeiQia
//{
//    [MCCore initWithAppkey:MEIQIA_APPKEY expcetionDelegate:nil];
//    
...
//    NSString *testValue =  [UMOnlineConfig getConfigParams:@"test"];
    NSString *testValue =  [UMOnlineConfig getConfigParams:@"gesturePop"];
    DDLogDebug(@"testValue:%@", testValue);
}
...
//获取教师成长消息红点
- (BOOL)isCommunionConversationHasNewData
{
...
-(void)registerBottomCounterNotification
{
    ...
    //处理消息页的红点
    [self subscribeMultipleCountTypes:@[@(TXClientCountType_Checkin),@(TXClientCountType_Notice),@(TXClientCountType_LearnGarden),@(TXClientCountType_GardenPost),@(TXClientCountType_Rest),@(TXClientCountType_JSB)] refreshBlock:^(NSArray *values) {
            ...
            TXAsyncRunInMain(^{
                ...
                if ([msgItem.badgeValue isEqualToString:@""]) {
                    ...
                    if ([self isPostConversationHasNewData]) {
                        ...
                        [msgItem setBadgeValue:@" "];
                        [msgItem layoutIfNeeded];
                    }else if ([self isCommunionConversationHasNewData]) {
                        //读取教师成长的红点
                        [msgItem setBadgeValue:@" "];
                        [msgItem layoutIfNeeded];
                    }else{
...
//        NSLog(@"亲子圈values:%@",values);
    ...
    NSArray *array = @[@{@"name":TX_PROFILE_KEY_OPTION_ANNOUNCEMENT, @"type":@(TXClientCountType_Announcement)},
                      ...
                      @{@"name":TX_PROFILE_KEY_OPTION_NOTICE, @"type":@(TXClientCountType_Notice)}];
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
	...
	[[TXChatClient sharedInstance].dataReportManager turnTimerOnOff:false];
}
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
	...
	[[TXChatClient sharedInstance].dataReportManager turnTimerOnOff:true];
}
...
-(void)handleReceiveRemoteNotification:(NSDictionary*)userInfo
{
    ...
    switch (type) {
        ...
        case TXPBPushTypePushLerngarden:
        {
            ...
            [self.window showAlertViewWithMessage:@"理解孩子有新内容，是否打开?" andButtonItems:cancel, confirm,nil];
        }
            break;
        default:
            break;
...
{
    if ([url.absoluteString hasPrefix:@"wjyteacher"])
    {
        NSLog(@"调用的应用程序的Bundle ID是: %@", sourceApplication);
        NSLog(@"URL scheme:%@", [url scheme]);
        NSLog(@"URL query: %@", [url query]);
        [self checkInInfo:url.absoluteString];
        return YES;
    }
    return [UMSocialSnsService handleOpenURL:url];
...
-(void)checkInInfo:(NSString *)urlStr
{
    if ([urlStr hasPrefix:@"wjyteacher://"] && [NSString isCardInfo:urlStr])
    {
        NSString *userId = [NSString getUserIdByUrl:urlStr];
        NSString *userName = [NSString getUserNameByUrl:urlStr];
        NSString *cardNumber = [NSString getUserCardNumberByUrl:urlStr];
        ...
        [self.window showAlertViewWithMessage:showTitle andButtonItems:cancel, confirm,nil];
    }
}
