...
void CCAppDelegate::setupMainScene()
{
    // initialize director
...
//    args[1] = std_string_to_jsval(cx, "gameId=10002");//
//    ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(sc->getGlobalObject()), "enterScene", 2, args);
...
void CCAppDelegate::executeJSFunctionWithParam(std::string param,const char * jsFunName){
	ScriptingCore* sc = ScriptingCore::getInstance();
	JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
	jsval args[1];
	args[0] = std_string_to_jsval(cx, param);
	ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(sc->getGlobalObject()), jsFunName, 1, args);
}
