...
- (void)selectPhoto{
	...
	if (!isPreview) {
		if ([placeholder.backgroundColor isEqual:[[UIColor blackColor] colorWithAlphaComponent:.1]]) {
			if (ASSETHELPER.selectdPhotos.count>=(ASSETHELPER.maxSelectionNumber - ASSETHELPER.currentSelectedCount)) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"reachMaxPhotoNumber" object:nil];
				return;
			}
...
- (void)setDelegate:(id)delegate{
    ...
    int page = floor((photosView.contentOffset.x - pageWidth / 2) / pageWidth) + 1;
	currentPage = page;
    ASSETHELPER.currentBrowerPage = page;
