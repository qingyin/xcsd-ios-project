        ...
        if (self.options & EMSDWebImageDownloaderIgnoreCachedResponse && responseFromCached) {
            ...
            if (CGSizeEqualToSize(image.size, CGSizeZero)) {
                ...
                if (self.expectedSize > 0) {
                    const NSInteger imgDataSize = self.imageData.length;
                    if (self.expectedSize != imgDataSize) {
//                        DDLogDebug(@"下载img大小不匹配,expectedSize:%@ imgSize:%@",@(self.expectedSize),@(imgDataSize));
                        NSLog(@"下载img大小不匹配,expectedSize:%@ imgSize:%@",@(self.expectedSize),@(imgDataSize));
                    }
                }
